...
const TextChat: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0B1120] relative transition-colors duration-300">
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto p-8 space-y-8 scroll-smooth pb-40 custom-scrollbar">
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-full max-w-4xl px-4 z-20">
        ...
        <div className="bg-white dark:bg-[#1E293B] rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50 border border-slate-200 dark:border-slate-700 p-1.5 flex items-end gap-2 transition-colors">
            <input 
                ...
                className="flex-1 bg-transparent border-0 py-3 px-4 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-0 text-base"
                autoFocus
            />
            <button 
                onClick={() => sendMessage(input)}
                disabled={!input.trim() || isLoading}
                className="p-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-colors disabled:bg-slate-200 disabled:text-slate-400 dark:disabled:bg-slate-700 dark:disabled:text-slate-500"
            >
